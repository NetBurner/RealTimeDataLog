...
class IntrospecObject
{
    IntroSpecDescription *pDesc = nullptr;
...
class START_MARKER_OBJ
{
   public:
    ...
    START_MARKER_OBJ()
    {
        if (IntrospecObject::bShowMe) IntrospecObject::Element(this, sizeof(*this), nullptr, 0);
    };
};
...
#define END_INTRO_OBJ                                    \
    uint32_t end_marker;                                 \
    virtual void GetExtent(uint32_t *&sm, uint32_t *&em) \
    {                                                    \
        sm = &start_marker.pItem;                        \
        em = &end_marker;                                \
    };                                                   \
    }
...
class INTROIEL
{
   public:
    T val;
    INTROIEL(const char *label)
    {
        val = 0;
        if (IntrospecObject::bShowMe) IntrospecObject::Element(this, sizeof(*this), label, 's');
    };
...
class INTROUEL
{
   public:
    T val;
    INTROUEL(const char *label)
    {
        val = 0;
        if (IntrospecObject::bShowMe) IntrospecObject::Element(this, sizeof(*this), label, 'u');
    };
    inline T operator=(T rhs)
    {
        val = rhs;
...
class INTROFEL
{
   public:
    T val;
    INTROFEL(const char *label)
    {
        val = 0;
        if (IntrospecObject::bShowMe) IntrospecObject::Element(this, sizeof(*this), label, 'f');
    };
    inline T operator=(T rhs)
    {
        val = rhs;
