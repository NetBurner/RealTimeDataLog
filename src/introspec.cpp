...
void IntrospecObject::Log(bool bForceLog)
{
    ...
    if (!pDesc) { pDesc = BuildOrFindDescription(m_name); }
    uint32_t *pstart;
    uint32_t *pend;
    GetExtent(pstart, pend);
    pstart++;
...
int LogEXRead(int fd, char *buf, int nbytes)
{
    return 0;
}
int LogEXClose(int fd)
{
    return 0;
}
...
void LogAppRecords()   // might not work without AppUpdate and config_obj
{
    IoExpandStruct ioe;
    ioe.read = LogEXRead;
